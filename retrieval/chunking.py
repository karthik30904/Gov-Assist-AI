...
def chunk_scheme(scheme: Scheme) -> list[SchemeChunk]:
    ...
    if not chunks:
        overview_parts = [
            ...
            f"Category: {scheme.category}" if scheme.category else "",
            f"State: {scheme.state}" if scheme.state else "",
            f"Ministry: {scheme.ministry}" if scheme.ministry else "",
            f"Description: {scheme.metadata.get('description')}" if scheme.metadata.get("description") else "",
            f"Application link: {scheme.application_link}" if scheme.application_link else "",
        ]
        text = "\n".join(part for part in overview_parts if part).strip()
        if text:
            chunk_id = hashlib.sha1(f"{scheme.source_url}:overview:{text}".encode("utf-8")).hexdigest()
            chunks.append(
                SchemeChunk(
                    chunk_id=chunk_id,
                    scheme_name=scheme.scheme_name,
